...
object BaseUtil {
  ...
  def getHdfs(url : String): FileSystem = {
    if (hdfs == null) hdfs = FileSystem.get(URI.create(url),new Configuration)
    hdfs
  }
}
...
object HdfsUtil {
  ...
  def listChildren(hdfs: FileSystem, fullName: String, holder: ListBuffer[String]): ListBuffer[String] = {
    ...
    for (status <- filesStatus) {
      ...
      if (isFile(hdfs, filePath)) holder += filePath.toString
      else listChildren(hdfs, filePath.toString, holder)
    }
    holder
  }
  ...
  def copyFile(hdfs: FileSystem, source: String, target: String): Unit = {
    ...
    if (!exists(hdfs, targetPath)) createFile(hdfs, targetPath)
  ...
  def copyFolder(hdfs: FileSystem, sourceFolder: String, targetFolder: String): Unit = {
    ...
    for (child <- children) copyFile(hdfs, child, child.replaceFirst(sourceFolder, targetFolder))
  }
  ...
  def copyFolderFromLocal(hdfs: FileSystem, localSource: String, hdfsTarget: String): Unit = {
    ...
    for (child <- allChildren) {
      ...
      if (child.isFile) copyFileFromLocal(hdfs, fullName, targetFileFullName)
      else copyFolderFromLocal(hdfs, fullName, targetFileFullName)
    }
  }
